...
class SimplexSolver():
    ...
    def update_enter_depart(self, matrix):
        ...
        for i in range(0, len(matrix[0])):
            if i < len(self.A[0]):
                ...
                self.entering.append("x_%s" % (str(i + 1)))
            else:
                self.entering.append("b")
        #print("ENTER ", self.entering);

        for i in range(1 + len(A[0]) - len(A), 1 + len(A[0])):
    ...
    def create_tableau(self):
        ...
        for i in range(0,len(self.tableau)):
            for j in range(0,len(self.tableau[0])):
                if j+1<len(self.tableau[0]):
                    zj[j]+= c_b[str(self.departing[i])]*self.tableau[i][j];
                else:
                    elem+= c_b[str(self.departing[i])]*self.b[i]

        for index, value in enumerate(c):
            c[index] = zj[index]-value
        self.tableau.append(c + [elem])
...
if __name__ == '__main__':
    ...
    # This is the Day 8 problem, with 4th and 5th columns swapped. so swap(x4,x5).
    A= [[1,-2,1, 0, 1,0,0],
        [-4,1,2, -1, 0,1,0],
        [-2,0,1, 0, 0,0,1]]
    b= [11, 3, 1]
    c= [0,0,0,0,0,-1,-1]    #Artificial Obj Func: Max z= -x6-x7.
